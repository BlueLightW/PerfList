...
import 'package:perf_list/add_anime.dart';
...
class _CreateListScreenState extends State<CreateListScreen> {
  ...
  void deleteSelected() {
    for(dynamic item in selecionadas) {
      ...
      .doc(userUid)
      .collection('listas')
      .doc('animes').collection('animes').doc(item).delete().then((_)=>print('lista removida: $item'));
  ...
  // Future<List<String>> _pegarListasCriadas() async {
  ...
  //         .doc('animes')
  //         .collection('animes')
  //          // Aqui você pode usar o nome da coleção que contém as listas
  //         .get();
...
Future<void> _atualizarLista(String listaId, String novoNome) async {
  ...
  Future<int> _pegarNumeroItens(doc) async {
    ...
    try {
      final snapshot = await firestore
          ...
          .collection('animeList')
          .get();
      print('numero de animes para a lista $doc: ${snapshot.docs.length}');
  ...
  Future<void> criarListaPersonalizada(String nomeLista) async {
    ...
    try {
      await firestore
          ...
          .collection('listas')
          .doc('animes')
          .collection('animes') // Usa o nome da lista como o nome da coleção
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('Criar Lista de Animes'),centerTitle: true,),
      body: Padding(
                        ...
                        builder: (context, snapshot) {
                          ...
                          return ListTile(
                            ...
                            trailing: snapshot.connectionState == ConnectionState.waiting
                                ...
                                : snapshot.hasError
                                    ? const Text('Erro')
                                    : Text('${snapshot.data ?? 0} animes'),
                            onTap: () {
                              (isSelected) ? toggleSelection(listasCriadas[index]['id'])
                              : Navigator.push(
                                context,
                                MaterialPageRoute(
                                  builder: (context) => AddAnimeScreen(lista: listasCriadas[index]['id']),
                                ),
                              );
                            },
                            selected: isSelected,
