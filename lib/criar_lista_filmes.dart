...
import 'package:perf_list/add_filme.dart';

class CreateListFilmesScreen extends StatefulWidget {
  const CreateListFilmesScreen({super.key});

  @override
  _CreateListFilmesScreenState createState() => _CreateListFilmesScreenState();
}

class _CreateListFilmesScreenState extends State<CreateListFilmesScreen> {
  ...
  void deleteSelected() {
    for(dynamic item in selecionadas) {
      ...
      .doc(userUid)
      .collection('listas')
      .doc('filmes').collection('filmes').doc(item).delete().then((_)=>print('lista removida: $item'));
...
Future<void> _atualizarLista(String listaId, String novoNome) async {
  ...
  Future<int> _pegarNumeroItens(doc) async {
    ...
    try {
      final snapshot = await firestore
          ...
          .collection('filmeList')
          .get();
      print('numero de filmes para a lista $doc: ${snapshot.docs.length}');
  ...
  Future<void> criarListaPersonalizada(String nomeLista) async {
    ...
    try {
      await firestore
          ...
          .collection('listas')
          .doc('filmes')
          .collection('filmes') // Usa o nome da lista como o nome da coleção
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('Criar Lista de Filmes'),centerTitle: true,),
      body: Padding(
                        ...
                        builder: (context, snapshot) {
                          ...
                          return ListTile(
                            ...
                            trailing: snapshot.connectionState == ConnectionState.waiting
                                ...
                                : snapshot.hasError
                                    ? const Text('Erro')
                                    : Text('${snapshot.data ?? 0} filmes'),
                            onTap: () {
                              (isSelected) ? toggleSelection(listasCriadas[index]['id'])
                              : Navigator.push(
                                context,
                                MaterialPageRoute(
                                  builder: (context) => AddFilmeScreen(lista: listasCriadas[index]['id']),
                                ),
                              );
                            },
                            selected: isSelected,
