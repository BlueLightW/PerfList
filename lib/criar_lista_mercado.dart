...
import 'package:perf_list/add_mercado.dart';

class CreateListMercadosScreen extends StatefulWidget {
  const CreateListMercadosScreen({super.key});

  @override
  _CreateListMercadosScreenState createState() => _CreateListMercadosScreenState();
}

class _CreateListMercadosScreenState extends State<CreateListMercadosScreen> {
  ...
  void deleteSelected() {
    for(dynamic item in selecionadas) {
      ...
      .doc(userUid)
      .collection('listas')
      .doc('mercados').collection('mercados').doc(item).delete().then((_)=>print('lista removida: $item'));
  ...
  Future<int> _pegarNumeroItens(doc) async {
    ...
    try {
      final snapshot = await firestore
          ...
          .collection('mercadoList')
          .get();
      print('numero de mercados para a lista $doc: ${snapshot.docs.length}');
  ...
  Future<void> criarListaPersonalizada(String nomeLista) async {
    ...
    try {
      await firestore
          ...
          .collection('listas')
          .doc('mercados')
          .collection('mercados') // Usa o nome da lista como o nome da coleção
                        ...
                        builder: (context, snapshot) {
                          ...
                          return ListTile(
                            ...
                            trailing: snapshot.connectionState == ConnectionState.waiting
                                ...
                                : snapshot.hasError
                                    ? const Text('Erro')
                                    : Text('${snapshot.data ?? 0} itens'),
                            onTap: () {
                              (isSelected) ? toggleSelection(listasCriadas[index]['id'])
                              : Navigator.push(
                                context,
                                MaterialPageRoute(
                                  builder: (context) => AddMercadoScreen(lista: listasCriadas[index]['id']),
                                ),
                              );
                            },
                            selected: isSelected,
